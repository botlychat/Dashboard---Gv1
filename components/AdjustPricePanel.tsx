...
const AdjustPricePanel: React.FC<AdjustPricePanelProps> = ({ date, units, getDailyPrice, onClose, onSave }) => {
    ...
    return (
        <div className="space-y-6">
             ...
             <div className="max-h-96 overflow-y-auto space-y-3 p-2 border rounded-md bg-gray-50">
                {units.map(unit => {
                    const defaultPrice = getDailyPrice(unit, date);
                    const overriddenPrice = priceOverrides[unit.id];

                    return (
                        <div key={unit.id} className="flex items-center justify-between p-3 bg-white rounded-md shadow-sm">
                            <div>
                                <p className="font-medium text-gray-800">{unit.name}</p>
                                <p className="text-xs text-gray-500">Default: {currencySymbol}{defaultPrice}</p>
                            </div>
                            <div className="flex items-center space-x-2">
                                <span className="font-semibold text-gray-500">{currencySymbol}</span>
                                <input
                                    type="number"
                                    value={overriddenPrice ?? ''}
                                    onChange={(e) => handleChange(unit.id, e.target.value)}
                                    placeholder={defaultPrice.toString()}
                                    className="w-32 p-2 text-right border rounded-md"
                                />
                            </div>
                        </div>
                    );
