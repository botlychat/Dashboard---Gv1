...
const BookingDetailsPanel: React.FC<BookingDetailsPanelProps> = ({ booking, unit, onClose, onCancelBooking }) => {
    ...
    if (!unit) return <div className="p-6">Unit information not available.</div>;
    ...
    return (
        <div>
            <div className="space-y-6">
                 <div className="space-y-6">
                    <InfoBlock title="Client & Unit">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <InfoItem icon="fa-user" label="Client Name" value={booking.clientName} />
                            <InfoItem icon="fa-building" label="Unit Name" value={unit.name} />
                        </div>
                    </InfoBlock>
                    
                    <InfoBlock title="Booking Information">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <InfoItem icon="fa-calendar-check" label="Check-in" value={checkIn.toLocaleDateString()} />
                            <InfoItem icon="fa-calendar-times" label="Check-out" value={checkOut.toLocaleDateString()} />
                            <InfoItem icon="fa-moon" label="Duration" value={`${nights} night${nights > 1 ? 's' : ''}`} />
                            <InfoItem icon="fa-info-circle" label="Status" value={<span className={`px-2 py-1 text-xs font-semibold rounded-full ${statusConfig.bgColor} ${statusConfig.textColor}`}>{statusConfig.label}</span>} />
                            <InfoItem icon="fa-hand-pointer" label="Booking Source" value={booking.bookingSource || 'N/A'} />
                            <InfoItem icon="fa-credit-card" label="Payment Method" value={booking.paymentMethod || 'N/A'} />
                        </div>
                        {booking.notes && <InfoItem icon="fa-sticky-note" label="Notes" value={<p className="whitespace-pre-wrap">{booking.notes}</p>} />}
                    </InfoBlock>
                </div>
            ...
            <div className="flex justify-end pt-6 mt-6 border-t dark:border-gray-700 space-x-2">
                 <button onClick={onClose} className="px-4 py-2 text-sm font-medium border rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 dark:border-gray-600">Close</button>
                 {canBeCancelled && (
                    <button onClick={() => onCancelBooking(booking.id)} className="px-4 py-2 text-sm font-medium text-white bg-red-600 rounded-md hover:bg-red-700">Cancel Reservation</button>
                 )}
            </div>
        </div>
    );
