...
const navItems = [
  ...
  { to: '/calendar', icon: 'fa-calendar-days', label: 'Calendar' },
  { to: '/units', icon: 'fa-building', label: 'Units' },
  { to: '/website-settings', icon: 'fa-globe', label: 'Website Settings' },
  { to: '/ai-agent', icon: 'fa-robot', label: 'AI Agent' },
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-gray-800 text-white transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 lg:flex-shrink-0 select-none`}>
        <div className="flex items-center justify-center h-20 border-b border-gray-700">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-xl font-semibold">Riyadh Getaways</span>
          </div>
        </div>
        <nav className="mt-4 px-2">
          {navItems.map((item) => (
            ...
            >
              <i className={`fa-solid ${item.icon} w-6 h-6 mr-3`}></i>
              {item.label}
            </NavLink>
          ))}
        </nav>
        <div className="absolute bottom-0 w-full p-4 text-center text-gray-500 text-xs">
          <p>&copy; {new Date().getFullYear()} Riyadh Getaways</p>
