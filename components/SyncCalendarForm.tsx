    ...
    return (
        <div className="space-y-8">
            <form onSubmit={handleSubmit} className="p-4 border rounded-lg dark:border-gray-700 space-y-4">
                <h3 className="text-lg font-medium text-gray-900 dark:text-white">{t(editingId ? 'editCalendarSync' : 'addCalendarSync')}</h3>
                ...
                <div>
                 ...
                 <div className="flex justify-end space-x-2">
                    {editingId && <button type="button" onClick={resetForm} className="px-4 py-2 text-sm font-medium border rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 dark:border-gray-600">{t('cancelEdit')}</button>}
            ...
            <div>
                <h3 className="text-lg font-medium text-gray-900 dark:text-white">{t('savedCalendars')}</h3>
                <div className="mt-4 space-y-2">
                    {externalCalendars.length > 0 ? (
                        externalCalendars.map(cal => (
                            <div key={cal.id} className="flex flex-wrap items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-md gap-2">
                                <div className="flex-grow">
                                    ...
                                    <p className="text-sm text-gray-500 dark:text-gray-400">{t('unit')}: {getUnitName(cal.unitId)}</p>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">{t('noCalendarsSynced')}</p>
                    )}
                </div>
            </div>
